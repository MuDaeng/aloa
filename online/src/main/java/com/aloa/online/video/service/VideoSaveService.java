...
import com.aloa.online.configuration.security.oauth.SignedInUserValidator;
...
public class VideoSaveService {
    ...
    private final SignedInUserValidator signedInUserValidator;
    ...
    public VideoRegisterResultDTO registerVideo(@Valid VideoRegisterDTO videoRegisterDTO){
        ...
        var videoMapping = VideoMapping.createVideoMapping(googleUserId);

        if(!signedInUserValidator.isUserOfVideo(youtubeVideo.channelId())) throw new IllegalArgumentException("다른사람의 영상입니다.");
    ...
    public void changeCharacter(@NonNull VideoMappingDTO videoMappingDTO) {
        if(!signedInUserValidator.isUserOfVideo(videoMappingDTO.getVideoId())) throw new IllegalArgumentException("비디오 정보가 정확하지 않습니다.");
